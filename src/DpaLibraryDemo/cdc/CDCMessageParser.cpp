...
static std::string createErrorString(const char* cause, int errorCode) {
...
CDCMessageParser::CDCMessageParser() {
    ...
    if (pthread_mutex_init(&csUI, NULL) != 0) {
        std::string errStr = createErrorString("Initialization of mutex failed", errno);
        throw CDCMessageParserException(errStr.c_str());
    }
}
...
CDCMessageParser::~CDCMessageParser() {
    ...
    if (pthread_mutex_destroy(&csUI) != 0) {
        std::cerr << "Destroy of mutex failed: " << errno << "\n";
    }
}

ParseResult CDCMessageParser::parseData(ustring& data) {
    pthread_mutex_lock(&csUI);

    ParseResult parseResult = implObj->parseData(data);

    pthread_mutex_unlock(&csUI);
    return parseResult;
}
